...
public class FollowHandler implements HttpHandler {
	...
	public String handleDeleteRequest(String[] pathElements, HttpExchange exchange) {
		try {
			if (pathElements.length == 3) {
				userController.removeConnection(JWTController.verifyToken(exchange), pathElements[2]);
	...
	public String handlePostRequest(String[] pathElements, HttpExchange exchange) {
		try {
			if (pathElements.length == 3) {
				userController.addConnection(JWTController.verifyToken(exchange), pathElements[2]);
	...
	public String handleGetRequest(String[] pathElements) {
		try {
			if (pathElements.length == 3) {
				return userController.getConnections(pathElements[2]);
			}
			else {
				throw new IOException("Path is not valid");
			}
